...
Integration testing combines all the software modules and tests them as a group. It's purpose is to verify both functional and non-functional requirements. The test cases focus less on individual object behaviour and more on the system as a whole.
...
The project was designed to question the backup industry and explore the possibility of provide a peer-based system that is easy to use and allows for file recovery. Throughout the project we build a proof-of-concept for exactly that, harnessing the cheap high-speed internet and redundant storage that many computer users have access to.

We began the project by creating an outline for Membrane structuring it into five distinct modules and setting out the expected challenges during the development process, this step guided the literature review, from which we drew inspiration for the design.

Reflecting on the literature review, we find that because the final form of Membrane was not set in stone we wander through the domains of both storage systems and agent based technology. Although we used concepts from intelligent agents, we find that practically, the reasoning system developed is much simpler than anticipated and a lot of original research was redacted.

By taking a pragmatic approach to development and design, we find that sticking to the strict guidelines mandated by intelligent agent technology, although neat in theory can add huge amounts of complexity to the system. An example of this is ontology development which offers great flexibility in reasoning for agents, however, a successful system that is easy to debug and understand can be designed by using simple protocols.

We also find that technologies researched, such as TCP hole punching, while very useful in theory may fall short in practise due to limitations of tools, especially when faced with time constrains and working at a higher level of abstraction. As a result we ended up using seemingly inferior options such as UPnP which requires support from the gateway.

After the literature review we proceed to the analysis, during which we planned the design and structure of Membrane before starting development. We first questioned users which was an insightful way to gather requirements. In retrospect, it would have also been useful to discuss the final requirements with the polled users.

Although all key requested requirements were fulfilled we find that some aspects of the application, namely the GUI was a far more desirable feature than first shown by polls, and a fully fledged GUI for interacting with Membrane was requested multiple times. In the future we will look at creating the requirements using an iterative approach, going back to the users after the initial poll.

Looking at features of existing solutions, specifically looking at how they differ proved an extremely valuable way on scoping Membrane, in fact, many of the desirable features discovered during the user survey were reiterated during the common feature analysis.
